...
class Instrument:
    ...
    def from_name(cls, observatory, instrument_name):
        ...
        for inst in observatory.instruments:
            if (
                ...
                or inst["name"] == instrument_name_alias[instrument_name]
            ):
                return cls(
    ...
    def get_fwhm(self, wafer: str, band: str) -> Optional[float]:
        ...
        if band in self.beam_omega[wafer]:
            ...
            return (
                (self.beam_omega[wafer][band] * 1e-9 * 4 * np.log(2.0) / np.pi) ** 0.5
                * 180.0
                * 60.0
                / np.pi
            )
