...
def test_median_residual_empty(map_with_single_source):
    ...
    assert pointing_residual_generator.coeffs.coeffs_ra == 0 * u.arcsec
    assert pointing_residual_generator.coeffs.coeffs_dec == 0 * u.arcsec
...
def test_median_residual(map_with_sources):
    ...
    assert (
        abs(pointing_residual_generator.coeffs.coeffs_ra - ra_offset) < 0.25 * u.arcmin
    )
    assert (
        abs(pointing_residual_generator.coeffs.coeffs_dec - dec_offset)
        < 0.25 * u.arcmin
    )
