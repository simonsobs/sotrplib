...
def test_make_enmap_shape_and_type(sim_map_params):
    ...
    expected_npix = int(2 * sim_map_params['maps']["width_ra"] / sim_map_params['maps']["resolution"] * 60)
    # Allow for rounding
    assert abs(test_map.shape[-2] - expected_npix) < 5
    expected_npix = int(2 * sim_map_params['maps']["width_dec"] / sim_map_params['maps']["resolution"] * 60)
...
def test_make_enmap_wcs(sim_map_params):
    ...
    assert abs(test_map.wcs.wcs.cdelt[0]) == sim_map_params['maps']["resolution"] * arcmin / degree
    assert abs(test_map.wcs.wcs.cdelt[1]) == sim_map_params['maps']["resolution"] * arcmin / degree
...
def test_make_noise_map_seed_behavior(sim_map_params):
    ...
    noise_map = sim_maps.make_noise_map(test_map, map_noise_Jy=0.1, map_mean_Jy=0.0, seed=None)
    assert isinstance(noise_map, enmap.ndmap)
...
def test_photutils_sim_n_sources_invalid_input():
    ...
    with pytest.raises(ValueError):
        sim_maps.photutils_sim_n_sources(test_map, n_sources=1, min_flux_Jy=1.0, max_flux_Jy=0.5)
