...
For each candidate row:
  1. Register it as a *persistent, monitored* source in the real
     ...
     with no further action needed per-candidate.
  2. Sync the newly-registered socat source into lightcurvedb (via the
     existing `lightcurvedb-socat` console script), so a lightcurvedb
     Source row exists before the historical sweep starts producing flux
     measurements to attach to it.
...
def main():  # pragma: no cover
    ...
    parser.add_argument("--box-half-width", type=float, default=None)
    ...
    for batch in chunk(rows, args.batch_size):
        cmd = build_wrapper_command(args.wrapper_script, batch, out_dir, args)
    ...
    log.info(
        ...
        n_candidates=len(rows),
        n_jobs=len(rows) * n_bands * n_tubes,
    )
