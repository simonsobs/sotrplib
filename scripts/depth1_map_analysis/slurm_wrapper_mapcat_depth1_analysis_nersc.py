...
P.add_argument(
    ...
    action="store",
    required=True,
    help="Account to charge compute hours to.",
)
...
P.add_argument(
    "--scratch-dir",
    action="store",
    default=f"/pscratch/sd/{list(USER)[0]}/{USER}/scratch/",
    help="Location of scratch directory for users. Default: /pscratch/sd/[user first letter]/[user]/scratch/.",
)
...
P.add_argument(
    ...
    action="store",
    default="regular",
    help="Quality of service, can be `regular` or `debug` for example.",
)
...
def generate_slurm_header(
    jobname, user_account, qos, cpu_per_task, script_dir, slurm_out_dir
):
    slurm_header = f"""#!/bin/bash
#SBATCH --account={user_account}   ## allocation to charge compute hours to.
#SBATCH --job-name={jobname}       ## create a short name for your job
#SBATCH --qos {qos}                ## quality of service, default `regular`
#SBATCH --nodes=1                # node count
#SBATCH --ntasks=1               # total number of tasks across all nodes
#SBATCH --cpus-per-task={cpu_per_task}       # cpu-cores per task (>1 if multi-threaded tasks)
#SBATCH --mem-per-cpu=36G         # memory per cpu-core (4G is default)
...
export MAPCAT_DATABASE_NAME=/pscratch/sd/a/afoster/SO/mapcat.sqlite

if [ ! -e "socat.pickle" ]; then
    socat-act-fits -f ../PS_S19_f090_2pass_optimalCatalog.fits -o socat.pickle
fi
...
for band in args.bands:
    for tube in args.optics_tubes:
        ...
        slurm_text += f"srun -n {args.nserial} -c {args.ncores} sotrp -c {config_file} > {args.out_dir}{tube}_{band}_sotrp.log "
